 ...
 var $w = $(window),
     ...
     _nmObj = {
      ...
      close: function() {
       ...
       var self = this;
       this._unreposition();
       self._callAnim('hideCont',
           function() {
            self._callAnim('hideLoad',
                function() {
                 self._callAnim('hideBg',
                     function() {
      ...
      _setCont: function(html, selector) {
       if (selector) {
        ...
        if (cur.length) {
         html = cur.html().replace(/<pre class="?nyroModalScript"? rel="?([0-9]*)"?><\/pre>/gi,
             function(x, y, z) {
              return tmp[y];
     ...
     _filters = {
      basic: {
       ...
       initElts: function(nm) {
        ...
        if (nm.closeOnClick) nm.elts.bg.off('click.nyroModal').on('click.nyroModal',
            function(e) {
             e.preventDefault();
             nm.close();
            });
       ...
       beforeShowCont: function(nm) {
        ...
        }).end().find('.nyroModalClose').on('click.nyroModal',
            function(e) {
             e.preventDefault();
             nm.close();
            });
   ...
   filledContent: function(nm) {
    ...
    if (link) {
     $('<a />', {
      text: 'previous',
      href: '#'
     }).addClass('nyroModalPrev').on('click',
         function(e) {
          e.preventDefault();
          nm._callFilters('galleryPrev');
         }).appendTo(append);
    ...
    if (link) {
     $('<a />', {
      text: 'next',
      href: '#'
     }).addClass('nyroModalNext').on('click',
         function(e) {
          e.preventDefault();
          nm._callFilters('galleryNext');
         }).appendTo(append);
...
jQuery(function($, undefined) {
 $.nmFilters({
  link: {
   ...
   init: function(nm) {
    nm.loadFilter = 'link';
    nm.opener.off('click.nyroModal').on('click.nyroModal',
        function(e) {
         e.preventDefault();
...
jQuery(function($, undefined) {
 $.nmFilters({
  form: {
   ...
   init: function(nm) {
    nm.loadFilter = 'form';
    nm.opener.off('submit.nyroModal').on('submit.nyroModal',
        function(e) {
         e.preventDefault();
         nm.opener.trigger('nyroModal');
        });
