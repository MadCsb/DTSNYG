...
(function($){
	...
	$.checkPassword=function(){
		...
		if(passwordInput.attr("value")!=checkInput.attr("value") &&options.checkInfoText!=""){
			checkInfoText.html("新密码与确认密码不匹配，请重新输入！");
	...
	var options2={
		...
		$.fn.password2=function(params){
			...
			$.passwordStrength2();
			//5.核对事件
			$("#"+options2.checkInput).bind("keyup",$.checkPassword2);
			$("#"+options2.passwordInput).bind("keyup",$.checkPassword2);
		}
	 
		$.passwordStrength2=function(){
			if(document.getElementById(options2.passwordInput)==null)
				return;
			$("#"+options2.passwordInput).bind("keyup",$.checkStrength2);
		}
		$.checkPassword2=function(){
			var passwordInput = $("#"+options2.passwordInput);
			var checkInfoText = $("#"+options2.checkInfoText);
			var passwordInput = $("#"+options2.passwordInput);
			var checkInput = $("#"+options2.checkInput);
			if(passwordInput.attr("value").length == 0 || checkInput.attr("value").length == 0)
				return false;
			if(passwordInput.attr("value").length<options2.minLength){
				checkInfoText.html("密码长度小于最小长度"+options2.minLength+"，请重新输入！");
				checkInfoText.addClass("pw_info_error");
				checkInfoText.removeClass("pw_info_right");
				return false;
			}
			if(passwordInput.attr("value")!=checkInput.attr("value") &&options2.checkInfoText!=""){
				checkInfoText.html("新密码与确认密码不匹配，请重新输入！");
				checkInput.addClass("input_error");
				checkInfoText.addClass("pw_info_error");
				checkInfoText.removeClass("pw_info_right");
		...
		$.checkStrength2=function(){
			var value = $('#'+options2.passwordInput).attr("value");
			var score=$.getValue(value);
			//分数怎么和表现关联
			var text=null;
			var color=null;
			...
			if(score<0){
				...
				color="gray";
				barLength='0%';
			}else if(score>=0 && score<options2.scores[0]){
				text=options2.verdects[0];
				color=options2.colors[0];
				barLength='33%';
			}else if(score>=options2.scores[0] && score<options2.scores[1]){
				text=options2.verdects[1];
				color=options2.colors[1];
				barLength='66%';
			}else if(score>=options2.scores[1]){
				text=options2.verdects[2];
				color=options2.colors[2];
				barLength='100%';
			}
			if(options2.strengthInfoText!="")
				$("#"+options2.strengthInfoText).html(text).css({
					color:color
				});
			if(options2.strengthInfoBar!="")
				$("#"+options2.strengthInfoBar).css({
					width:barLength,
					backgroundColor:color
				});
		}
		$.getValue2=function(_value){
			var score = 0;
			var num=$.countCharNum2(_value);
			// 如果是普通密码则设置为0
			for(var i=0;i<options2.common.length;i++){
				if(_value==options2.common[i])
					return 0;
			}
			if(_value.length<options2.minLength)
				return -100;
			else
				score+=(_value.length-options2.minLength)*2;
			score+=num*2;
			// 有小写字母有数字
			if(_value.match(/[a-z]/) && _value.match(/\d+/)){score+=5}
			// 有大写字母有数字
		...
		$.checkParams2=function(){
			if(document.getElementById(options2.checkInput)==null || 
				document.getElementById(options2.passwordInput)==null)
				return false;
			return true;
		}
		$.mustInput2=function(){
			if(!options2.mustInput)return;
			$("#"+options2.passwordInput).after("<span class='mustinput'>&nbsp;</span>");
			$("#"+options2.checkInput).after("<span class='mustinput'>&nbsp;</span>");
		}
		$.getOptions2=function(params){
			for(var property in params){
				options2[property]=params[property];
			}
		}
})(jQuery)