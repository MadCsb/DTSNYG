...
options.onAjaxFormComplete(true, form, "", options);
		...
		_validateField : function(field, options, skipAjaxValidation) {
			...
			for ( var i = 0; i < rules.length; i++) {
				...
				case "equals":
					...
					break;
				case "morethan":
					errorMsg = methods._morethan(field, rules, i, options);
					break;
				case "lessthan":
		 ...
		 * 
		 */
		_morethan : function(field, rules, i, options) {
			var morethanField = rules[i + 1];
			var morethanName = rules[i + 2];
			var flag = rules[i + 3];
			var morethanVal = $("#" + morethanField).val();
			if (flag == 1) {
				if (morethanVal.length > 0
						&& parseFloat(field.val()) <= parseFloat(morethanVal)) {
					return options.allrules.morethan.alertText + morethanName
							+ options.allrules.morethan.alertText2;
				}
			} else {
				if (morethanVal.length > 0
						&& parseFloat(field.val()) < parseFloat(morethanVal)) {
					return "* 请输入大于等于" + morethanName + "的值";
				}
		...
		_lessthan : function(field, rules, i, options) {
			...
			var lessthanName = rules[i + 2];
			var flag = rules[i + 3];
			var lessthanVal = $("#" + lessthanField).val();
			if (flag == 1) {
				if (lessthanVal.length > 0
						&& parseFloat(field.val()) >= parseFloat(lessthanVal)) {
					return options.allrules.lessthan.alertText + lessthanName
							+ options.allrules.lessthan.alertText2;
				}
			} else {
				if (lessthanVal.length > 0
						&& parseFloat(field.val()) > parseFloat(lessthanVal)) {
					return "* 请输入小于等于" + lessthanName + "的值";
				}
			}
		},
		/**
		...
		_maxSize : function(field, rules, i, options) {
			...
			for ( var i = 0; i < field.val().length; i++) {
				code = field.val().charCodeAt(i);// 获取当前字符的unicode编码
				if (code > 255)// 在这个unicode编码范围中的是所有的英文字母已经各种字符
				{
					codeLen = codeLen + 2;
				} else {
					codeLen++;
	...
	// horizontally
	// topRight:20, -15 means topRight position shifted by 20 pixels to right
	// and 15 pixels to top
...
_saveOptions : function(form, options) {
	...
	$.validationEngine = {
		defaults : {
			...
			// Name of the event triggering field validation "blur"
