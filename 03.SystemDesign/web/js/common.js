...
function login_dialog(){
    CUR_DIALOG = ajax_form('login','您还未登录',MALLURL+'/index.php?app=login&inajax=1',400,1);
}
	       	...
	       	if(result.cart_goods_num >0){
	            ...
	            for (var i = 0; i < result['list'].length; i++){
	            	...
	            	html+='<dd class="goods-thumb"><a href="'+goods['goods_url']+'" title="'+goods['goods_name']+'"><img src="'+goods['goods_image']+'"></a></dd>';
		          	...
		          	html+='<dd class="handle"><a href="javascript:void(0);" onClick="drop_topcart_item('+goods['cart_id']+','+goods['gid']+','+goods['sld_is_supplier']+');">删除</a></dd>';
		          	html+="</dl>";
		        }
		        obj.find('.incart-goods').html(html);
	         	html = "共<i>"+result.cart_goods_num+"</i>种商品&nbsp;&nbsp;共计&nbsp;<em>&yen;"+result.cart_all_price+"</em>";
...
function drop_topcart_item(cart_id,gid,is_supplier){
    ...
    $.getJSON(MALLURL+'/index.php?app=cart&mod=del&cart_id='+cart_id+'&gid='+gid+'&is_supplier=is_supplier&callback=?', function(result){
        if(result.state){
            var obj = $('.head-user-menu .my-cart');
            //删除成功
            if(result.quantity == 0){
                ...
                $('div[bbcType="rtoolbar_total_price"]').html("共计金额：<em class='goods-price'>&yen;"+result.amount+"</em>");
...
(function($) {
    $.show_bbc_login = function(options) {
        ...
        login_dialog_html.append('<form class="sld_bg" method="post" id="ajax_login" action="'+APP_SITE_URL+settings.action+'"></form>').find('form')
            .append('<div class="login_inajax_title">账户登录</div>')
        	...
        	.append('<dl><dd><input type="text" placeholder="用户名" name="user_name" autocomplete="off" class="text"></dd></dl>')
        	.append('<dl><dd><input type="password" placeholder="密码" autocomplete="off" name="password" class="text"></dd></dl>')
        	.append('<dl><dd><input placeholder="验证码" type="text" size="10" maxlength="5" class="text fl w60 inajax_captcha" name="captcha"><img border="0" onclick="this.src=\''+APP_SITE_URL+'/index.php?app=randcode&rand='+Math.random()+';\'" name="codeimage" title="看不清，换一张" src="'+APP_SITE_URL+'/index.php?app=randcode&rand='+Math.random()+';" class="fl ml10 inajax_img"><span></span></dd></dl>')
        	.append('<div class="enter"><input type="submit" name="Submit" value="登录" class="submit"></div><input type="hidden" name="ref_url" value="'+ref_url+'">')
            .append('<ul><li class="forget_password_li"><a href='+APP_SITE_URL+'"/index.php?app=login&mod=forget_password" class="forget">忘记密码？</a></li><li class="go_register_li">没有帐号？<a href='+APP_SITE_URL+'"/index.php?app=login&mod=register" class="register">立即注册>></a></li></ul>');
 ...
 *
 * HTML part
 * <a href="javascript" bbctype="mcard" data-param="{'id':5}"></a>
 *
 * JAVASCRIPT part
 ...
 * $('a[bbctype="mcard"]').membershipCard();
 */
(function($){
	$.fn.membershipCard = function(options){
		                		...
		                		if(data.follow != 2){
		                			...
		                			if(data.follow == 1){
		                				$('<div class="follow-handle" bbctype="follow-handle'+data.id+'" data-param="{\'mid\':'+data.id+'}"></div>')
		                					...
		                					.append('<a href="javascript:void(0);" bbctype="nofollow">取消关注</a>').find('a[bbctype="nofollow"]').click(function(){
		                						onfollow($(this));
		                					}).end().appendTo(_bottom);
		                			}else{
		                				$('<div class="follow-handle" bbctype="follow-handle'+data.id+'" data-param="{\'mid\':'+data.id+'}"></div>')
		                					.append('<a href="javascript:void(0);" bbctype="follow">加关注</a>').find('a[bbctype="follow"]').click(function(){
		                						follow($(this));
		                					}).end().appendTo(_bottom);
		                			}
		                			$('<div class="send-msg"> <a href="'+MALLURL+'/index.php?app=home&mod=sendmsg&member_id='+data.id+'" target="_blank"><i></i>站内信</a> </div>').appendTo(_bottom);
		                		}
		...
		function follow(o){
			...
			$.getJSON(MALLURL+'/index.php?app=myfriend&mod=addfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
					$('[bbctype="follow-handle'+data_str.mid+'"]').html('<a href="javascript:void(0);" >已关注</a> <a href="javascript:void(0);" bbctype="nofollow">取消关注</a>').find('a[bbctype="nofollow"]').click(function(){
						onfollow($(this));
					});
				}
			});
		...
		function onfollow(o){
			...
			$.getJSON(MALLURL+'/index.php?app=myfriend&mod=delfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
					$('[bbctype="follow-handle'+data_str.mid+'"]').html('<a href="javascript:void(0);" bbctype="follow">加关注</a>').find('a[bbctype="follow"]').click(function(){
						follow($(this));
					});
				}
			});
