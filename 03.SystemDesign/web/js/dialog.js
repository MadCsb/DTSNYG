...
function hostconvert(url) {
    if((!(url.match(/^http:\/\//)||url.match(/^https:\/\//)))) url = COMMON_URL.replace(/(\/+)$/g, '') + '/' + url;
...
function ajaxupdateevent(o) {
	if(typeof o == 'object' && o.getAttribute) {
		if(o.getAttribute('ajaxtarget')) {
			...
			_attachEvent(o, ajaxevent, newfunction('ajaxget', ajaxurl, o.getAttribute('ajaxtarget'), o.getAttribute('sld_byuTP9yr'), o.getAttribute('ajaxloading'), o.getAttribute('ajaxdisplay')));
...
function ajaxpost(formid, showid, waitid, showidclass, submitbtn, recall) {
	...
	var showidclass = '';//showidclass = !showidclass ? '' : showidclass;
...
function showloading(display, waiting) {
	...
	$$('sld_byuTP9yr').innerHTML = waiting;
	$$('sld_byuTP9yr').style.display = display;
}
...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	...
	s += t ? t : '<span class="dialog_title_icon">提示</span>';
	...
	if(mode == 'info') {
		...
		s += '<i class="' + (mode == 'alert' ? 'alert_error' : (mode == 'succ' ? 'alert_right' : 'alert_info')) + '"></i></br><span class="tipmsg">' + msg + '</span></div></div>';
